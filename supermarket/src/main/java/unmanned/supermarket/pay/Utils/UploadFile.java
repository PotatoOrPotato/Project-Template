...
import unmanned.supermarket.pay.enums.FileEnum;
...
public class UploadFile {
    ...
    // 上传图片
    public static String[] imagesType = { "gif", "jpeg", "png", "jpg", "tif", "bmp" };
    // 上传文档
    public static String[] othersType = { "html", "htm", "doc", "xls", "txt", "zip", "rar", "pdf", "cll" };
    ...
    public static String getNewFileName(String fileName, String newName, String nullSuffix) {
        String suffix = getSuffixAndDot(fileName);
        if (suffix != null) {
            newName += suffix;
        } else {
    ...
    /**
     * 删除某个目录下所有文件（不包含文件夹，包含文件夹下的文件）
     * @param path
     * @return
     */
    public static boolean removeAllFile(String path) {
        return removeAllFile(new File(path));
    }
     ...
     * @return 是否成功
     */
    public static boolean removeFileAndFolder(String path) {
        return removeFileAndFolder(new File(path));
    }
     ...
     * @return 删除是否成功
     */
    public static boolean removeNowFolder(String path) {
        return removeNowFolder(new File(path));
    }
    ...
    public static boolean isImageType(String fileName) {
        return isTypeByFileName(fileName, imagesType);
    }

    /**
     * 验证当前文件是否是非图片类型（常用办公文件类型）
     * othersType 可以设置文件类型
     * @param fileName 验证文件的名称
     * @return 是否合法
     */
    public static boolean isFileType(String fileName) {
        return isTypeByFileName(fileName, othersType);
    }
     ...
     * @return 是否合法
     */
    public static boolean isTypeByFileName(String fileName, String[] allowTypes) {
        return isTypeByFileName(fileName, allowTypes, true);
    }
     ...
     * @return 是否合法
     */
    public static boolean isImageTypeBySuffix(String suffix) {
        return isTypeByFileNameBySuffix(suffix, imagesType);
    }

    /**
     * 验证当前后缀、文件类型是否是非图片类型（常用办公文件类型）
     * othersType 可以设置文件类型
     * @param suffix 验证文件的后缀
     * @return 是否合法
     */
    public static boolean isFileTypeBySuffix(String suffix) {
        return isTypeByFileNameBySuffix(suffix, othersType);
    }


    /**
     * 根据文件后缀名和类型数组，验证文件类型是否合法
     * @param suffix 后缀名
     * @param allowTypes 类型数组
     * @return 是否合法
     */
    public static boolean isTypeByFileNameBySuffix(String suffix, String[] allowTypes) {
        return isTypeByFileNameBySuffix(suffix, allowTypes, true);
    }
     ...
     * @throws Exception
     */
    public static FileEnum upload4Stream(String fileName, String path, InputStream is) throws Exception {
        init();
        FileEnum state = FileEnum.UPLOAD_FAILURE;
        ...
        try {
            ...
            state = FileEnum.UPLOAD_SUCCSSS;
        } catch (FileNotFoundException e) {
            state = FileEnum.UPLOAD_NOTFOUND;
            throw e;
        } catch (IOException e) {
            state = FileEnum.UPLOAD_FAILURE;
            throw e;
        } finally {
            if (is != null) {
                is.close();
     ...
     * @throws Exception
     */
    public static FileEnum upload4Stream(String fileName, String path, File file) throws Exception {
        init();
        FileEnum state = FileEnum.UPLOAD_FAILURE;
        ...
        try {
            ...
            if (size <= 0) {
                state = FileEnum.UPLOAD_ZEROSIZE;
            } else {
                if (size <= maxFileSize) {
                    fis = new FileInputStream(file);
                    state = upload4Stream(fileName, path, fis);
                } else {
                    state = FileEnum.UPLOAD_OVERSIZE;
                }
            }
        } catch (FileNotFoundException e) {
            state = FileEnum.UPLOAD_NOTFOUND;
     ...
     * @throws Exception
     */
    public static FileEnum upload4Stream(String fileName, String path, File file, String[] allowTypes) throws Exception {
        FileEnum state = FileEnum.UPLOAD_FAILURE;
        if (isTypeByFileName(fileName, allowTypes)) {
            state = upload4Stream(fileName, path, file);
        } else {
            state = FileEnum.UPLOAD_TYPE_ERROR;
        }
        return state;
    }
     ...
     * @throws Exception
     */
    public static FileEnum upload4Stream(String fileName, String path, InputStream fs, String[] allowTypes) throws Exception {
        FileEnum state = FileEnum.UPLOAD_FAILURE;
        if (isTypeByFileName(fileName, allowTypes)) {
    ...
    /**
     * 利用FileUtils上传文件；其中maxFileSize是限制上传文件的大小
     * @param fileName 文件名称
     * @param path 保存路径
     * @param file 文件
     * @return 是否上传成功
     ...
     * @return 是否验证通过
     */
    public static boolean isTypeByFileName(String fileName, String[] allowTypes, boolean flag) {
    ...
    /**
     * 根据后缀和类型数组验证文件类型是否合法，flag是否区分后缀大小写，true严格大小写
     * @author hoojo
     ...
     * @return 是否合法
     */
    public static boolean isTypeByFileNameBySuffix(String suffix, String[] allowTypes, boolean flag) {
        boolean valid = false;
        if (allowTypes.length > 0 && "*".equals(allowTypes[0])) {
            valid = true;
        } else {
     ...
     * @return 是否删除成功
     */
    public static boolean removeNowFolder(File file) {
        boolean flag = false;
    ...
    /**
     * 删除某个文件夹下的所有文件（除文件夹外），包含子文件夹里的文件
    ...
    /**
     * 传入一个文件名(abc.doc)，得到这个文件名称的后缀(.doc)
     * @param fileName 文件名
     * @return 后缀名
     */
    public static String getSuffixAndDot(String fileName) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println(getDateName("a.jpg"));
        System.out.println(getDateName("a.jpg"));
        System.out.println(getNewFileName("a", "bbb", "txt"));
        System.out.println(getUUIDName("a.htm"));
        System.out.println(getSuffixAndDot("a.jpg"));
        ...
        System.out.println("#############################################");
        System.out.println("###" + isTypeByFileName("a", new String[]{"*"}));
        System.out.println("###" + isTypeByFileName("a.JPG", imagesType));
        System.out.println("###" + isTypeByFileName("a.JPG", imagesType, false));
        System.out.println(isTypeByFileNameBySuffix("cals", new String[]{"*", "b"}));
        System.out.println(isTypeByFileNameBySuffix("b", new String[]{"cal", "B"}, false));
    }
}